...
from custom_layernorm import (
    CustomLayerNormAutograd,
    CustomLayerNormManualFunction,
    CustomLayerNormManualModule,
)
...
class TestLayerNorm(unittest.TestCase):
    def test_autograd(self):
        ...
        for test_num in range(10):
            ...
            y_auto = bn_auto(x)
            self.assertLess(
                np.max(np.abs(y_auto.mean(dim=1).data.numpy())), 1e-5
            )
            self.assertLess(
                np.max(np.abs(y_auto.var(dim=1).data.numpy() - 1)), 1e-1
            )
    ...
    def test_manual_function(self):
        ...
        for test_num in range(5):
            ...
            self.assertLess(
                np.max(
                    np.abs(
                        y_manual_fct.mean(dim=1).data.numpy()
                        - beta.mean().item()
                    )
                ),
                1e-5,
    ...
    def test_manual_module(self):
        ...
        for test_num in range(10):
            ...
            self.assertLess(
                np.max(np.abs(y_manual_mod.mean(dim=1).data.numpy())), 1e-5
            )
            self.assertLess(
                np.max(np.abs(y_manual_mod.var(dim=1).data.numpy() - 1)), 1e-1
            )
